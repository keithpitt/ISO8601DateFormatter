...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
	...
	if (strict && isspace(*ch)) {
		...
		unsigned i = 0U;
		for(unsigned len = (unsigned int)strlen((const char *)ch); i < len; ++i) {
			if (!isspace(ch[i]))
				break;
		}
		...
		if (*ch == 'T') {
			...
			year = (unsigned int)nowComponents.year;
			month_or_week = (unsigned int)nowComponents.month;
			day = (unsigned int)nowComponents.day;
		} else {
			segment = 0U;
				...
				case 2:
					switch(num_leading_hyphens) {
						...
						case 1:; //-YY; -YY-MM (implicit century)
							...
							unsigned current_year = (unsigned int)nowComponents.year;
